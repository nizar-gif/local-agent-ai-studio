        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
            <div className="bg-card border border-border rounded-lg w-full max-w-2xl flex flex-col max-h-[90vh]">
                ...
                <div className="flex-1 p-6 space-y-6 overflow-y-auto">
                    ...
                    <section>
                        ...
                        <div className="space-y-4">
                            <SelectField label="Destination Folder" options={['/ (root)', '/reports', '/code']} />
                            <SelectField label="Assign Category" options={['(None)', 'Knowledge', 'Support', 'Code', 'Policy']} />
                            <Checkbox label="Auto-vectorize after upload" defaultChecked />
                            <Checkbox label="Overwrite existing files" />
                            
                            <div>
                                <button onClick={() => setShowOverrides(!showOverrides)} className="text-sm text-text-secondary hover:text-text-primary">
                                    {showOverrides ? 'Hide' : 'Show'} RAG Parameter Overrides
                                </button>
                                {showOverrides && (
                ...
                <footer className="p-4 border-t border-border flex justify-end gap-4">
                    ...
                    <button className="bg-primary hover:bg-primary-hover text-white px-6 py-2 rounded-md font-semibold">Upload & Ingest</button>
                </footer>
            </div>
        </div>
    );
